...
describe("Request", function () {
    ...
    describe("isFresh", function () {
        ...
        it("is not fresh for ONCE if httpIfModifiedSince is not set", function () use ($now) {
            $request = new Request(["c" => "o", "e" => "foo"], [], $now);
            expect($request->isFresh)->toBe(false);
        });
        it("is fresh for ONCE if httpIfModifiedSince less than 0", function () use ($now, $httpAncient) {
            $request = new Request(["c" => "o", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $httpAncient], $now);
            expect($request->isFresh)->toBe(true);
        });
        it("is fresh for ONCE if httpIfModifiedSince is 0", function () use ($now, $httpTimeStamp0) {
            $request = new Request(["c" => "o", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $httpTimeStamp0], $now);
            expect($request->isFresh)->toBe(true);
        });
        it("is fresh for ONCE if httpIfModifiedSince greater 0", function () use ($now, $httpNow) {
            $request = new Request(["c" => "o", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $httpNow], $now);
            expect($request->isFresh)->toBe(true);
        });

        it("is not fresh for DAILY if lastlog is not set", function () use ($now) {
            $request = new Request(["c" => "d", "e" => "foo"], [], $now);
            expect($request->isFresh)->toBe(false);
        });
        it("is not fresh for DAILY if lastlog is older than a day", function () use ($now, $http25HoursAgo) {
            $request = new Request(["c" => "d", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $http25HoursAgo], $now);
            expect($request->isFresh)->toBe(false);
        });
        it("is fresh for DAILY if lastlog is not older than a day", function () use ($now, $http23HoursAgo) {
            $request = new Request(["c" => "d", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $http23HoursAgo], $now);
            expect($request->isFresh)->toBe(true);
        });

        it("is not fresh for MONTHLY if lastlog is not set", function () use ($now) {
            $request = new Request(["c" => "m", "e" => "foo"], [], $now);
            expect($request->isFresh)->toBe(false);
        });
        it("is not fresh for MONTHLY if lastlog is older than a month", function () use ($now, $http32DaysAgo) {
            $request = new Request(["c" => "m", "e" => "foo"], ["HTTP_IF_MODIFIED_SINCE" => $http32DaysAgo], $now);
            expect($request->isFresh)->toBe(false);
        });
        it("is fresh for MONTHLY if lastlog is not older than a month", function () use ($now, $http27DaysAgo) {
